...
impl Moving2D {
    pub fn new(address: DmxAddress, dmx_tx: mpsc::Sender<(DmxAddress, DmxValue)>) -> Moving2D {
        Moving2D {
            address: address,
            x: 0,
            y: 0,
    ...
    pub fn fade_simple(&mut self, curve: FadeCurve, time: FadeTime, end_x: DmxValue, end_y: DmxValue) {
        ...
        for (&x, &y) in get_fade_steps_int(self.x, end_x, steps, curve.clone()).iter().zip(get_fade_steps_int(self.y, end_y, steps, curve.clone()).iter()) {
            self.dmx_tx.send((self.address + 0, x)).unwrap();
            self.dmx_tx.send((self.address + 1, y)).unwrap();
            self.x = x;
            self.y = y;
