...
impl RGBA {
    ...
    //         let mut channel_a_locked = channel_a.lock().expect("Failed to lock Arc!");
    ...
    pub fn fade_simple(&mut self, curve: FadeCurve, time: FadeTime, end_r: DmxValue, end_g: DmxValue, end_b: DmxValue, end_a: DmxValue, kill_others: bool) {
        ...
        let (tx, rx) = mpsc::channel();
        if try_stop_fades(vec!(&self.channel_r, &self.channel_g, &self.channel_b, &self.channel_a), tx, kill_others) {
            let channel_r = self.channel_r.clone();
            let channel_g = self.channel_g.clone();
            let channel_b = self.channel_b.clone();
            let channel_a = self.channel_a.clone();
